...
| Agent 基建 | AGENTS.md、MCP、Skills 这些长期标准是什么 | 原理、标准、可长期引用 |
...
- 除非已经明确存在外部商业依赖、投放链接或合同级文档引用，否则长期结构正确性优先于短期链接连续性。

这条规则的目的，是避免内容仓库为了历史命名长期背兼容包袱。公开文档不是不可变 API，IA 应该服务当前理解和长期维护边界。
