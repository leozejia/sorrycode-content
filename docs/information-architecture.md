...
图片能力按供应商页面维护，不在“开始使用”中另建图片总览。可执行的 Skill 另有独立入口，但不复制模型页的 API 细节：

- `GPT Image 2` 和 `GPT Image 2.5` 分别维护各自的模型与参数边界，同时承接 Codex 自然语言生图和显式 Images API。`SorryCode Image2` 只维护安装、Skill 专用 Key、触发、落盘和边界，模型页与 Skill 页互相链接；
