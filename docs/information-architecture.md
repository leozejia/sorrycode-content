...
| Platform | SorryCode API、Key、计费和平台入口怎么选 | 平台能力和开发者参考 |
| 模型与工作台 | 不同供应商生态的工作台、模型和媒体能力怎么用 | 按 OpenAI / Anthropic / xAI 分组，执行路径必须闭环 |
...
### Platform、模型与工作台和 Agent 基建

`Platform` 写 SorryCode 平台能力。`模型与工作台` 写具体供应商生态的工作台、模型和
媒体能力。`Agent 基建` 写长期标准和协议。
...
图片能力按供应商页面维护，不再放在 `Platform / 生成图片`：
...
`Runtime / GPT Image 2`。更多素材生产和视觉资产工作流仍可指向 `sorryassets.com`，
