...
| Platform | SorryCode API、Key、模型和图片入口怎么选 | 平台能力和开发者参考 |
...
1. 默认方式是在 Codex App 对话中直接生成和迭代图片。
2. 需要编辑本地图片、固定输出目录、指定尺寸或保存诊断时，使用
   `SorryCode Image2` Skill；该 Skill 当前只使用 `gpt-image-2`。

不要把原始 Images HTTP API 写成第三条新手路径，也不要在 SorryCode 公共内容中
维护其他 AIGC 模型清单。更多 AIGC 模型、素材生产和视觉资产工作流统一指向
`sorryassets.com`，但不在 SorryCode 文档里展开其模型、价格或接口。
