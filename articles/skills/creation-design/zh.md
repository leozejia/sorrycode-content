...
这类产物通常需要几轮修改。先得到可编辑的初稿，再指出质量、使用场景或个人偏好上的具体问题。
...
第一次只是想做一套分享稿，可以直接使用[藏师傅的 PPT Skill](/docs/skills/magazine-web-ppt)。

<h2 id="iterate">怎么修改初稿</h2>

| 问题 | 怎么反馈 |
| --- | --- |
| 字太小、对比不足、图片模糊、排版溢出 | 直接指出页面和问题，让 Agent 修到可用 |
| 不适合投影、客户阅读或当前品牌 | 补充受众、场合、品牌和交付限制 |
| 想更克制、更有冲击力或接近某个参考 | 给出方向或参考，只修改相关部分 |

如果还说不清哪里有问题，可以先让 Agent 审查：

```text
请先不要修改。检查这份初稿，把问题分成质量、使用场景和个人偏好三类。每类只列最重要的三个问题，再问我先改哪一类。
```

需要局部调整时，明确页面范围和必须保留的内容：

```text
不要重做整套。只调整第 3 到 6 页，提高投影可读性，保留现有结构，减少装饰元素。改完后列出修改过的页面。
```
...
```text
请读取 SorryCode Image2 文档，生成一张公众号封面，主题是 AI 编程入门，中文标题清晰，保存到 outputs/images/cover/。
```
...
如果你只是第一次尝试图片生成，先看 [SorryCode Image2](/docs/skills/sorrycode-image2)；需要手动调用 Images API 时，再看 [GPT Image 2](/docs/runtime/gpt-image-2)。需要文档成品时再装 [Kami](/docs/skills/kami)。需要演讲展示时，再看 [演示 / PPT 交付路线](#presentation-route)；需要代码式 PPT 或视频工具链时，再看 [Open Slide](/docs/tools/open-slide) 和 [HyperFrames](/docs/tools/hyperframes)。
