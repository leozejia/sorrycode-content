...
slug: dsh
order: 1
summary: 启动 DeepSeek Harness Web UI，通过自定义 Responses provider 接入 SorryCode，并完成首条模型请求。
section: runtime
...
DSH 不只支持 DeepSeek 模型。它可以添加自定义 OpenAI-compatible provider，因此也能通过 SorryCode 使用 `grok-4.6` 等 Responses 模型。
...
然后打开 [API Key 页面](https://sorrycode.com/keys)，选择一把所属分组包含目标模型的 Key。本文用 `grok-4.6` 演示，因此需要选择包含该模型的 Grok 分组 Key。使用其他模型时，改用对应分组的 Key。
...
<h2 id="configure">添加 SorryCode provider</h2>
...
Provider ID 需要使用小写，并且会写入会话和模型记录。保存后不要随意改名；需要另一个协议时，新增一个 Provider ID。

点击`获取可用模型`，DSH 会使用当前 Base URL 和 Key 查询 `/models`。从返回结果中选择 `grok-4.6`，再保存 provider。如果模型发现失败，但 Key 和 Base URL 已确认正确，也可以手动添加准确的模型 ID。
...
1. 在模型选择器中选择 `sorrycode / grok-4.6`
2. 点击`选择工作区`
...
DSH 的自定义 provider 不绑定 Grok。要使用其他 Responses 模型：

1. 选择包含目标模型的 SorryCode Key
2. 保持 Base URL 为 `https://sorrycode.com/v1`
3. 保持 API 协议为 `openai-responses`
4. 从`获取可用模型`的结果中选择准确模型 ID

一个 provider route 只能使用一种 API 协议。如果以后必须使用 Chat Completions，请新建另一个 Provider ID，不要把两种协议混在同一路由中。
