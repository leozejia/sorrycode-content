...
稳定做法是先安装并读取 [SorryCode Image2](/docs/skills/sorrycode-image2)，然后在 Codex App 中说：

```text
请使用 SorryCode Image2，生成一张中文播客封面，主题是 AI 编程，新手友好，暖色调，干净排版，并保存图片结果。
```

Agent 按文档调用 Images API，保存并检查图片结果。生成后可以继续用自然语言调整构图、比例、颜色和文字区域。
