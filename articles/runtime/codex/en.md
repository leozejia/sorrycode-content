...
This is the default path. The console generates an install command for the current API key. Paste that command into the terminal on your own computer, and the installer will connect `Codex` to `{{API_BASE_URL}}` and write the local config. After install, use `Codex App` to open your project.

### Step 1: Copy the Command in the Console

1. Open `https://sorrycode.com/keys`
2. Find the API key you prepared for Codex
...
The one-click installer handles the dependencies needed for the Codex setup path. On macOS, it prepares `Node.js 22 LTS` and `Codex CLI`; you do not need to learn Homebrew or Git first. Windows uses the Windows-specific installer. If you are not sure what PowerShell is, read [Environment / Windows PowerShell](/docs/environment/windows-powershell) first.
