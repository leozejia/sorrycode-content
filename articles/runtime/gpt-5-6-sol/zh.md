...
在 Codex 中使用 GPT-5.6 Sol，常见档位是 `low`、`medium`、`high`、`xhigh`、`max` 和 `ultra`。不同界面也可能把 `low` 和 `xhigh` 显示为 Light 和 Extra High。
...
| `ultra` | 保持 Max 推理深度，同时让多个 Agent 并行处理可拆分任务 |

从旧模型迁移时，先保留原来的 effort，再比较同一级和低一级。GPT-5.6 的 Token 效率更高，低一级也可能保持或提高质量。

<h2 id="max-ultra">Max 和 Ultra 怎么选</h2>

| 档位 | 它控制什么 |
| --- | --- |
| `max` | 让选中的模型用更多时间处理一个任务 |
| `ultra` | 使用 Max 推理深度，并增加主动的子 Agent 协作 |

Ultra 可以理解成 Max 的多 Agent 升级版。单个 Agent 的推理深度没有超过 Max，增加的是任务拆分和并行执行。它适合可以分成多个独立工作流的大任务；无法并行拆分的问题，继续使用 Max 更直接。

Ultra 不是名为 `gpt-5.6-sol-ultra` 的新模型。OpenAI API 提供 multi-agent beta，用来构建类似 Ultra 的体验；Codex 用户直接以当前界面显示的档位为准。

不要在提示词里写“请更努力思考”。档位通过 Codex 的模型选择器设置，提示词继续描述目标、上下文、约束、证据和输出格式。
